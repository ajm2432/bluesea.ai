...
export async function POST(req: Request) {
 ...
 const categoriesContext = USE_CATEGORIES
   ? `
   To help with our internal classification of inquiries, please categorize your responses in addition to answering them. We have provided you with **${customerSupportCategories.categories.length}** customer support categories:
   
   - ${categoryListString}

   If multiple categories match, include multiple category IDs in your response. If none match, return an empty array.
   `
   : "";

 // Change the system prompt for your use case
 const systemPrompt = `You are acting as an AI IT customer support assistant chatbot inside a chat window on a website. You are chatting with a human user who is asking for help about IT Support. When responding to the user, aim to provide concise and helpful responses while maintaining a polite and professional tone.
 ...
 In addition to using the provided information, you can draw upon general IT knowledge and common support scenarios to address user inquiries. If no specific information is available or relevant, you can provide general advice, troubleshooting steps, or direct the user to resources related to their inquiry.

 ${categoriesContext}

 If the question is unrelated to IT support, you should redirect the user to a human agent.
